...
def import_with_pybert_to_df(stg_path: Path):
    """
    Try to import STG file using pyBERT, pygimli, or fallback parser.
    Returns DataFrame with A,B,M,N,CURRENT,dV columns.
    """
    errs = []
    df = None
    source = None
    ...
    # 1) pyBERT (commented out as it's not available in this environment)
    # try:
    ...
    # 2) pygimli.ert.load (commented out as we're using mock)
    ...
    try:
        ...
        # Apply column coalescing to normalize names
